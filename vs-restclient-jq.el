;;; vs-restclient-jq.el --- Support for setting vs-restclient vars from jq expressions
;;
;; Public domain.
;;
...
;; Original Author: Cameron Dorrat <cdorrat@gmail.com>
;; Maintainer: Cameron Dorrat <cdorrat@gmail.com>
;; Created: 26 Apr 2020
;; Keywords: http jq
;; Package-Requires: ((vs-restclient "20200502.831") (jq-mode "0.4.1") (emacs "24.4"))
...
;;; Commentary:
;;
;; This is a companion to vs-restclient.el to add support for setting variables from results using jq expressions
...
;; --- jq support
(defun vs-restclient-jq-result-end-point ()
  (save-excursion
    (goto-char (point-max))
...
(defun vs-restclient-jq-get-var (jq-pattern)
  (with-temp-buffer
    (let ((output (current-buffer)))
      (with-current-buffer vs-restclient-same-buffer-response-name
        (call-process-region
         (point-min)
         (vs-restclient-jq-result-end-point)
         shell-file-name
         nil
         output
         nil
